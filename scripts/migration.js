...
function migrateTrackingFolderMetadataSchema(data)
{
	console.time('Migration: trackingFolderMetadataSchema');

	const folderMetadata = require(p.join(appDir, '.dist/tracking/folder-metadata.js'));
	const current = data.trackingFolderMetadata || {};
	const migrated = {};

	for(const key in current)
	{
		migrated[key] = folderMetadata.sanitizeFolderMetadata(current[key], Date.now());
	}

	data.trackingFolderMetadata = migrated;

	console.timeEnd('Migration: trackingFolderMetadataSchema');
